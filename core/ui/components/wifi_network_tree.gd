...
func _ready() -> void:
	...
	if not network_manager.is_running():
		print("WARN: NetworkManager is not running")
...
func network_connect(password: String, ssid: String) -> void:
	...
	if wifi_devices.is_empty():
		print("WARN: No wifi devices found")
		connecting = false
		return
	...
	if valid_access_points.is_empty():
		print("WARN: Unable to find access point with SSID:", ssid)
		connecting = false
		return
...
func refresh_networks() -> void:
	print("Refreshing wifi networks")
